 ...
 */
class theme_essential_fel_core_renderer extends core_renderer
{
    ...
    {
        $breadcrumbstyle = theme_essential_fel_get_setting('breadcrumbstyle');
    ...
    public function footer() {
        ...
        if (defined('MDL_PERF') || (!empty($CFG->perfdebug) and $CFG->perfdebug > 7)) {
            ...
            if (defined('MDL_PERFTOFOOT') || debugging() || $CFG->perfdebug > 7) {
                $performanceinfo = theme_essential_fel_performance_output($perf, theme_essential_fel_get_setting('perfinfo'));
            }
        }
        ...
        if ($CFG->debug) {
            if (empty($DEBUGCAUSE)) $DEBUGCAUSE = 'Standard Debug Mode';
    ...
    */
    ...
    public function custom_menu_courses() {
        ...
        $hasdisplaymycourses = theme_essential_fel_get_setting('displaymycourses');
        if (isloggedin() && !isguestuser() && $hasdisplaymycourses) {
            $mycoursetitle = theme_essential_fel_get_setting('mycoursetitle');
            if ($mycoursetitle == 'module') {
                $branchtitle = get_string('mymodules', 'theme_essential_fel');
            } else if ($mycoursetitle == 'unit') {
                $branchtitle = get_string('myunits', 'theme_essential_fel');
            } else if ($mycoursetitle == 'class') {
                $branchtitle = get_string('myclasses', 'theme_essential_fel');
            } else {
                $branchtitle = get_string('mycourses', 'theme_essential_fel');
            }
            ...
            if ($courses = enrol_get_my_courses(NULL, 'fullname ASC')) {
                foreach ($courses as $course) {
                    if (is_dir($CFG->dirroot.'/local/my')) {
                        ...
                        if ($config->skipmymetas) {
                            if (local_my_is_meta_for_user($course->id, $USER->id)) {
                                continue;
                            }
                        }
                    }
            ...
            if ($numcourses == 0 || empty($courses)) {
                $noenrolments = get_string('noenrolments', 'theme_essential_fel');
    ...
    public function custom_menu_themecolours()
    {
        ...
        if (!isguestuser()) {
            ...
            foreach (range(1, 3) as $alternativethemenumber) {
                if (theme_essential_fel_get_setting('enablealternativethemecolors' . $alternativethemenumber)) {
                    $alternativethemes[] = $alternativethemenumber;
                }
            }
            if (!empty($alternativethemes)) {
                $branchtitle = get_string('themecolors', 'theme_essential_fel');
                $branchlabel = '<i class="fa fa-th-large"></i>' . $branchtitle;
                $branchurl = new moodle_url('#');
                $branchsort = 300;
                $branch = $colourmenu->add($branchlabel, $branchurl, $branchtitle, $branchsort);

                $defaultthemecolorslabel = get_string('defaultcolors', 'theme_essential_fel');
                $branch->add('<i class="fa fa-square colours-default"></i>' . $defaultthemecolorslabel,
                    new moodle_url($this->page->url, array('essential_felcolours' => 'default')), $defaultthemecolorslabel);
                foreach ($alternativethemes as $alternativethemenumber) {
                    if (theme_essential_fel_get_setting('alternativethemename' . $alternativethemenumber)) {
                        $alternativethemeslabel = theme_essential_fel_get_setting('alternativethemename' . $alternativethemenumber);
                    } else {
                        $alternativethemeslabel = get_string('alternativecolors', 'theme_essential_fel', $alternativethemenumber);
                    }
                    $branch->add('<i class="fa fa-square colours-alternative' . $alternativethemenumber . '"></i>' . $alternativethemeslabel,
                        new moodle_url($this->page->url, array('essential_felcolours' => 'alternative' . $alternativethemenumber)), $alternativethemeslabel);
                }
            }
        }
    ...
    public function custom_menu_messages()
    {
        ...
        if (empty($totalmessages)) {
            ...
            $messagetitle = get_string('nomessagesfound', 'theme_essential_fel');
    ...
    protected function get_time_difference($created_time)
    {
        ...
        if (intval($time_difference / $years) > 1) {
            ...
            return get_string('ago', 'core_message', get_string('few', 'theme_essential_fel') . get_string('seconds'));
        }
    }
    ...
    public function custom_menu_user()
    {
        ...
        if (!isloggedin()) {
            ...
            // Render Help Link
            $usermenu .= $this->theme_essential_fel_render_helplink();
            ...
            if (\core\session\manager::is_loggedinas()) {
                ...
                $branchlabel = '<em><i class="fa fa-key"></i>' . fullname($realuser, true) . get_string('loggedinas', 'theme_essential_fel') . fullname($USER, true) . '</em>';
            ...
            if (is_mnet_remote_user($USER) && $idprovider = $DB->get_record('mnet_host', array('id' => $USER->mnethostid))) {
                $branchlabel = '<em><i class="fa fa-users"></i>' . get_string('loggedinfrom', 'theme_essential_fel') . $idprovider->name . '</em>';
            ...
            $usermenu .= $this->theme_essential_fel_render_preferences($context);
            ...
            if ($course->id == 1) {
                ...
                foreach ($hascourses as $hascourse) {
                    ...
                    if (has_capability('gradereport/user:view', $reportcontext) && $hascourse->visible) {
                        $branchlabel = '<em><i class="fa fa-list-alt"></i>' . get_string('mygrades', 'theme_essential_fel') . '</em>';
                ...
                $branchlabel = '<em><i class="fa fa-list-alt"></i>' . get_string('coursegrades', 'theme_essential_fel') . '</em>';
     ...
     * @return string
     */
    protected function theme_essential_fel_render_helplink()
    {
        global $USER;
        if (!theme_essential_fel_get_setting('helplinktype')) {
            return false;
        }
        ...
        if (theme_essential_fel_get_setting('helplinktype') == 1) {
            if (filter_var(theme_essential_fel_get_setting('helplink'), FILTER_VALIDATE_EMAIL)) {
                $branchurl = 'mailto:' . theme_essential_fel_get_setting('helplink') . '?cc=' . $USER->email;
            } else if ((theme_essential_fel_get_setting('helplink')) && (filter_var(get_config('supportemail'), FILTER_VALIDATE_EMAIL))) {
        ...
        if (theme_essential_fel_get_setting('helplinktype') == 2) {
            if (filter_var(theme_essential_fel_get_setting('helplink'), FILTER_VALIDATE_URL, FILTER_FLAG_SCHEME_REQUIRED)) {
                $branchurl = theme_essential_fel_get_setting('helplink');
                $target = '_blank';
            } else if ((!theme_essential_fel_get_setting('helplink')) && (filter_var(get_config('supportpage'), FILTER_VALIDATE_URL, FILTER_FLAG_SCHEME_REQUIRED))) {
     ...
     */
    protected function theme_essential_fel_render_preferences($context)
    {
        global $USER, $CFG;
        ...
        if (has_capability('moodle/user:editownmessageprofile', $context)) {
            $branchlabel = '<em><i class="fa fa-comments"></i>' . get_string('messagepreferences', 'theme_essential_fel') . '</em>';
            $branchurl = new moodle_url('/message/edit.php', array('id' => $USER->id));
            $preferences .= html_writer::tag('li', html_writer::link($branchurl, $branchlabel));
        }
        if ($CFG->enableblogs) {
            $branchlabel = '<em><i class="fa fa-rss-square"></i>' . get_string('blogpreferences', 'theme_essential_fel') . '</em>';
            $branchurl = new moodle_url('/blog/preferences.php');
            $preferences .= html_writer::tag('li', html_writer::link($branchurl, $branchlabel));
        }
        if ($CFG->enablebadges && has_capability('moodle/badges:manageownbadges', $context)) {
            $branchlabel = '<em><i class="fa fa-certificate"></i>' . get_string('badgepreferences', 'theme_essential_fel') . '</em>';
    ...
    */
    ...
    protected static function replace_moodle_icon($icon, $alt = false)
    {
        $icons = array(
            'add' => 'plus',
            'book' => 'book',
    ...
    {
        if (theme_essential_fel_get_setting($socialnetwork)) {
            $icon = $socialnetwork;
            ...
            $socialhtml .= html_writer::start_tag('button', array('type' => "button",
                ...
                'onclick' => "window.open('" . theme_essential_fel_get_setting($socialnetwork) . "')",
                'title' => get_string($socialnetwork, 'theme_essential_fel'),
            ));
