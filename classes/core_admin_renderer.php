...
class theme_essential_fel_core_admin_renderer extends core_admin_renderer {
    ...
    function admin_notifications_page($maturity, $insecuredataroot, $errorsdisplayed, $cronoverdue, $dbproblems,
        ...
        $str .= $OUTPUT->heading(get_string('technical', 'theme_essential_fel'));
    ...
    public function plugins_control_panel(core_plugin_manager $pluginman, array $options = array()) {
        ...
        if (is_dir($CFG->dirroot.'/local/adminsettings')) {
            $extracond = has_capability('local/adminsettings:nobody', $systemcontext);
        }
        ...
        foreach ($plugininfo as $type => $plugins) {
            ...
            foreach ($plugins as $name => $plugin) {
                ...
                $updateinfo = '';
                if (empty($CFG->disableupdatenotifications) && is_array($plugin->available_updates()) && $extracond) {
