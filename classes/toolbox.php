...
class toolbox {
    ...
    static public function get_csswww() {
        ...
        if (!self::lte_ie9()) {
            if (\right_to_left()) {
                $moodlecss = 'essential_fel-rtl.css';
            } else {
                $moodlecss = 'essential_fel.css';
            }

            $syscontext = \context_system::instance();
            $itemid = \theme_get_revision();
            $url = \moodle_url::make_file_url("$CFG->wwwroot/pluginfile.php",
                            "/$syscontext->id/theme_essential_fel/style/$itemid/$moodlecss");
            $url = preg_replace('|^https?://|i', '//', $url->out(false));
            return '<link rel="stylesheet" href="' . $url . '">';
        } else {
            if (\right_to_left()) {
                $moodlecssone = 'essential_fel-rtl_ie9-blessed1.css';
                $moodlecsstwo = 'essential_fel-rtl_ie9.css';
            } else {
                $moodlecssone = 'essential_fel_ie9-blessed1.css';
                $moodlecsstwo = 'essential_fel_ie9.css';
            }

            $syscontext = \context_system::instance();
            $itemid = \theme_get_revision();
            $urlone = \moodle_url::make_file_url("$CFG->wwwroot/pluginfile.php",
                            "/$syscontext->id/theme_essential_fel/style/$itemid/$moodlecssone");
            $urlone = preg_replace('|^https?://|i', '//', $urlone->out(false));
            $urltwo = \moodle_url::make_file_url("$CFG->wwwroot/pluginfile.php",
                            "/$syscontext->id/theme_essential_fel/style/$itemid/$moodlecsstwo");
    ...
    static public function get_setting($setting, $format = false) {
        ...
        if (!method_exists($us, 'get_setting')) {
            return null;
        }
    ...
    static public function setting_file_url($setting, $filearea) {
        ...
        if (!method_exists($us, 'setting_file_url')) {
            return null;
        }
    ...
    static public function course_content_search() {
        $canwe = false;
        global $CFG;
        if ((self::get_setting('coursecontentsearch')) && (file_exists("$CFG->dirroot/theme/essential_fel/"))) {
            $canwe = true;
        }
        return $canwe;
    }
    ...
    static private function check_corerenderer() {
        ...
        if (empty($us->corerenderer)) {
            ...
            global $OUTPUT;
            if (property_exists($OUTPUT, 'essential_fel')) {
                ...
                // Fallback check.
                if (($corerenderer != null) && (property_exists($corerenderer, 'essential_fel'))) {
                    ...
                    if (($corerenderer != null) && (property_exists($corerenderer, 'essential_fel'))) {
                        ...
                        $us->corerenderer = $PAGE->get_renderer('theme_essential_fel', 'core');
                    }
                }
            }
        }
    ...
    static public function render_indicators($numberofslides) {
        ...
        for ($indicatorslideindex = 0; $indicatorslideindex < $numberofslides; $indicatorslideindex++) {
            $indicators .= '<li data-target="#essential_felCarousel" data-slide-to="'.$indicatorslideindex.'"';
    ...
    static public function render_slide_controls() {
        ...
        $prev = '<a class="left carousel-control" href="#essential_felCarousel" data-slide="prev" aria-label="'.$strprev.'">';
        $prev .= '<span aria-hidden="true" class="fa fa-chevron-circle-left"></span></a>';
        $next = '<a class="right carousel-control" href="#essential_felCarousel" data-slide="next" aria-label="'.$strnext.'">';
    ...
    static public function initialise_colourswitcher(\moodle_page $page) {
        ...
        $page->requires->js_call_amd('theme_essential_fel/coloursswitcher', 'init',
    ...
    static public function set_headerbackground($css, $headerbackground) {
        ...
        if ($headerbackground) {
            $replacement .= 'background-size: cover;';
        }
    ...
    static public function set_modfixedwidth($css, $replacement) {
        $tag = '[[setting:modfixedwidth]]';
        $css = str_replace($tag, $replacement, $css);
        return $css;
    }
