...
echo $OUTPUT->essential_fel_blocks('page-top', 'row-fluid', 'aside', 'pagetopblocksperrow');
...
if (!$tablet) {
    if ((($hasboringlayout) && ($left)) || ((!$hasboringlayout) && (!$left))) {
        echo $OUTPUT->essential_fel_blocks('side-pre', 'span4 desktop-first-column');
    } else {
        echo $OUTPUT->essential_fel_blocks('side-pre', 'span4 pull-right');
    }
}
?>
            </div>
    ...
    echo $OUTPUT->essential_fel_blocks('side-pre', '');
    echo $OUTPUT->essential_fel_blocks('side-post', '');
?> </div></div> <?php
        } else {
            $postclass = 'span3';
            if (!$left) {
            ...
            echo $OUTPUT->essential_fel_blocks('side-post', $postclass);
        }
?>
    </div>
    <!-- End Main Regions -->
