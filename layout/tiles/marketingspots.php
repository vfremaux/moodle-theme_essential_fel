...
for ($mspot = 1; $mspot <= 3; $mspot++) {
    $spotimage[$mspot] = \theme_essential_fel\toolbox::get_setting('marketing'.$mspot.'image');
    if ($spotimage[$mspot]) {
        $additionalmarketingclass = true;
    }
    $spotbutton[$mspot] = $OUTPUT->essential_fel_marketing_button($mspot);
    if ($spotbutton[$mspot]) {
        ...
        <div class="marketing-spot span4">
            <div class="title"><h5><span>
                <span aria-hidden="true" class="fa fa-<?php
                    echo \theme_essential_fel\toolbox::get_setting('marketing'.$mspot.'icon'); ?>"></span>
                <?php echo \theme_essential_fel\toolbox::get_setting('marketing'.$mspot, true); ?>
            </span></h5></div>
...
?>
            <div class="content<?php echo ($additionalmarketingcontentclass) ? ' withbutton' : ''; ?>">
                <?php
                    echo \theme_essential_fel\toolbox::get_setting('marketing'.$mspot.'content', 'format_html');
