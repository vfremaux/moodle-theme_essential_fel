...
class essential_fel_admin_setting_configinteger extends admin_setting_configtext {
    ...
    public function validate($data) {
        if (!is_number($data)) {
            $validated = get_string('asconfigintnan', 'theme_essential_fel', array('value' => $data));
        } else {
            $validated = parent::validate($data); // Pass parent validation first.

            if ($validated == true) {
                if ($data < $this->lower) {
                    $validated = get_string('asconfigintlower', 'theme_essential_fel', array('value' => $data, 'lower' => $this->lower));
                } else if ($data > $this->upper) {
                     $validated = get_string('asconfigintupper', 'theme_essential_fel', array('value' => $data, 'upper' => $this->upper));
                } else {
                    $validated = true;
                }
            }
