 ...
 */
class theme_essential_fel_toolbox_testcase extends advanced_testcase {
    ...
    protected function setup_renderer() {
        ...
        $toolbox = \theme_essential_fel\toolbox::get_instance();
    ...
    public function test_get_tilefile_header() {
        $this->setup_renderer();
        $thefile = \theme_essential_fel\toolbox::get_tile_file('header');
        global $CFG;
        $withoutdirroot = str_replace($CFG->dirroot, '', $thefile);

        $this->assertEquals('/theme/essential_fel/layout/tiles/header.php', $withoutdirroot);
    }

    public function test_get_tilefile_pagesettings() {
        $this->setup_renderer();
        $thefile = \theme_essential_fel\toolbox::get_tile_file('pagesettings');
        global $CFG;
        $withoutdirroot = str_replace($CFG->dirroot, '', $thefile);

        $this->assertEquals('/theme/essential_fel/layout/tiles/pagesettings.php', $withoutdirroot);
    }

    public function test_get_tilefile_slideshow() {
        $this->setup_renderer();
        $thefile = \theme_essential_fel\toolbox::get_tile_file('slideshow');
        global $CFG;
        $withoutdirroot = str_replace($CFG->dirroot, '', $thefile);

        $this->assertEquals('/theme/essential_fel/layout/tiles/slideshow.php', $withoutdirroot);
    }

    public function test_render_indicators() {
        $this->setup_renderer();
        $theindicators = \theme_essential_fel\toolbox::render_indicators(4);
        $thecontent = '<li data-target="#essential_felCarousel" data-slide-to="0" class="active"></li>';
        $thecontent .= '<li data-target="#essential_felCarousel" data-slide-to="1"></li>';
        $thecontent .= '<li data-target="#essential_felCarousel" data-slide-to="2"></li>';
        $thecontent .= '<li data-target="#essential_felCarousel" data-slide-to="3"></li>';
        $this->assertEquals($thecontent, $theindicators);
    }

    public function test_render_slide() {
        set_config('slide1url', 'https://about.me/gjbarnard', 'theme_essential_fel');
        set_config('slide1target', '_blank', 'theme_essential_fel');
        set_config('slide1', 'Test slide one', 'theme_essential_fel');
        set_config('slide1caption', '<p>Test of link in caption: <a href="https://about.me/gjbarnard" target="_blank">me.</a></p>',
            'theme_essential_fel');
        set_config('slide2target', '_blank', 'theme_essential_fel');
        set_config('slide2', 'Test slide two', 'theme_essential_fel');
        ...
        $theslide1 = \theme_essential_fel\toolbox::render_slide(1, 0);
        ...
        $thecontent1 .= '<img src="http://www.example.com/moodle/theme/image.php/_s/essential_fel/theme/1/default_slide" ';
        $thecontent1 .= 'alt="Test slide one" class="carousel-image"></div></div></div></div></a>';
        $this->assertEquals($thecontent1, $theslide1);

        $theslide2 = \theme_essential_fel\toolbox::render_slide(2, 0);
        ...
        $thecontent2 .= '<img src="http://www.example.com/moodle/theme/image.php/_s/essential_fel/theme/1/default_slide" ';
    ...
    public function test_themeinfo() {
        set_config('theme', 'essential_fel');
        $this->resetAfterTest(true);
        $this->setup_renderer();
        ...
        $this->assertEquals('essential_fel', $PAGE->theme->name);
        $this->assertEquals('/theme/essential_fel', $themedir);
